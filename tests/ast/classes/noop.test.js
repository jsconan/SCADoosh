 ...
 *
 * Unit tests: AST node that represents an empty statement.
 *
...
describe('AstNoop', () => {
    ...
    it('should create an AstNoop', () => {
        ...
        expect(node).to.be.an.instanceOf(AstNoop);
    ...
    it('should clone an AstNoop', () => {
        ...
        expect(clone).to.be.an.instanceOf(AstNoop);
        expect(clone).to.not.be.equal(node);
        expect(clone).to.be.deep.equal(node);
    });

    it('should clone an AstNoop with the provided properties', () => {
        const value = 'foo';
        const node = (new AstNoop()).startAt(1, 1, 0).endAt(1, 2, 1);
