 ...
 *
 * Unit tests: AST node that represents an operator call.
 *
...
describe('AstOperatorCall', () => {

    it('should create an AstOperatorCall with a list of parameters and a list of statements', () => {
        const type = 'operator-call';
        ...
        const node = new AstOperatorCall(identifier, parameters, statements);

        expect(node).to.be.an('object');
        expect(node).to.be.an.instanceOf(AstNode);
        expect(node).to.be.an.instanceOf(AstFragment);
        expect(node).to.be.an.instanceOf(AstFunctionCall);
        expect(node).to.be.an.instanceOf(AstOperatorCall);
    ...
    it('should create an AstOperatorCall with a single parameter and a single statement', () => {
        const type = 'operator-call';
        const identifier = new AstIdentifier('foo');
    ...
    it('should create an AstOperatorCall with no parameter and no statement', () => {
        const type = 'operator-call';
        const identifier = new AstIdentifier('foo');
        const parameters = [];
        const statements = [];
    ...
    it('should create an AstOperatorCall with the provided properties', () => {
        const type = 'foo';
        const identifier = new AstIdentifier('foo');
        const parameters = [new AstNumber(1), new AstNumber(2)];
        const statements = [new AstAssignment(new AstIdentifier('foo'), new AstNumber(42))];
    ...
    it('should throw a TypeError if one of the operands is not a valid AstFragment', () => {
        expect(() => new AstOperatorCall(new AstIdentifier(1), [], {})).to.throw(TypeError);
        expect(() => new AstOperatorCall(new AstIdentifier(1), [], [{}])).to.throw(TypeError);
        expect(() => new AstOperatorCall(new AstIdentifier(1), [], [new AstNumber(1), {}])).to.throw(TypeError);
        expect(() => new AstOperatorCall(new AstIdentifier(1), [], new AstNode('foo'))).to.throw(TypeError);
        expect(() => new AstOperatorCall(new AstIdentifier(1), [], [new AstNode('foo')])).to.throw(TypeError);
        ...
        expect(() => new AstOperatorCall(new AstIdentifier(1), new AstNode('foo'), [])).to.throw(TypeError);
        expect(() => new AstOperatorCall(new AstIdentifier(1), [new AstNode('foo')], [])).to.throw(TypeError);
        expect(() => new AstOperatorCall({}, [], [])).to.throw(TypeError);
        expect(() => new AstOperatorCall(new AstNode('foo'), [], [])).to.throw(TypeError);
        expect(() => new AstOperatorCall(new AstNumber(2), [], [])).to.throw(TypeError);


        expect(() => new AstOperatorCall(new AstIdentifier('foo'), [], [], {identifier: {}})).to.throw(TypeError);
        expect(() => new AstOperatorCall(new AstIdentifier('foo'), [], [], {identifier: new AstNode('foo')})).to.throw(TypeError);
        expect(() => new AstOperatorCall(new AstIdentifier('foo'), [], [], {identifier: new AstNumber(1)})).to.throw(TypeError);
        expect(() => new AstOperatorCall(new AstIdentifier('foo'), [], [], {parameters: {}})).to.throw(TypeError);
        expect(() => new AstOperatorCall(new AstIdentifier('foo'), [], [], {parameters: [{}]})).to.throw(TypeError);
        expect(() => new AstOperatorCall(new AstIdentifier('foo'), [], [], {parameters: new AstNode('foo')})).to.throw(TypeError);
        expect(() => new AstOperatorCall(new AstIdentifier('foo'), [], [], {parameters: [new AstNode('foo')]})).to.throw(TypeError);
        expect(() => new AstOperatorCall(new AstIdentifier('foo'), [], [], {statements: {}})).to.throw(TypeError);
        expect(() => new AstOperatorCall(new AstIdentifier('foo'), [], [], {statements: [{}]})).to.throw(TypeError);
        expect(() => new AstOperatorCall(new AstIdentifier('foo'), [], [], {statements: new AstNode('foo')})).to.throw(TypeError);
        expect(() => new AstOperatorCall(new AstIdentifier('foo'), [], [], {statements: [new AstNode('foo')]})).to.throw(TypeError);
    });

    it('should stringify an AstOperatorCall', () => {
        const type = 'operator-call';
        const identifier = new AstIdentifier('foo');
        const parameters = [new AstNumber(1), new AstNumber(2)];
        const statements = [new AstAssignment(new AstIdentifier('foo'), new AstNumber(42))];
    ...
    it('should clone an AstOperatorCall', () => {
        const identifier = new AstIdentifier('foo');
        const parameters = [new AstNumber(1), new AstNumber(2)];
        const statements = [new AstAssignment(new AstIdentifier('foo'), new AstNumber(42))];
        const node = (new AstOperatorCall(identifier, parameters, statements)).startAt(1, 1, 0).endAt(1, 6, 5);
        ...
        expect(node).to.be.an.instanceOf(AstFunctionCall);
        expect(clone).to.be.an.instanceOf(AstOperatorCall);
        expect(clone).to.not.be.equal(node);
        expect(clone).to.be.deep.equal(node);
    });

    it('should clone an AstOperatorCall with empty list of properties', () => {
        const identifier = new AstIdentifier('foo');
        const parameters = [new AstNumber(1), new AstNumber(2)];
        const statements = [new AstAssignment(new AstIdentifier('foo'), new AstNumber(42))];
        const node = (new AstOperatorCall(identifier, parameters, statements)).startAt(1, 1, 0).endAt(1, 6, 5);
    ...
    it('should clone an AstOperatorCall with the provided properties', () => {
        const identifier = new AstIdentifier('foo');
        const parameters = [new AstNumber(1), new AstNumber(2)];
        const statements = [new AstAssignment(new AstIdentifier('foo'), new AstNumber(42))];
    ...
    it('should throw a TypeError if one of the operands is not a valid AstFragment when cloning', () => {
        const node = (new AstOperatorCall(new AstIdentifier(1), [], [])).startAt(1, 1, 0).endAt(1, 4, 3);
